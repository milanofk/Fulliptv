...
# Get distro data
. /etc/lsb-release >> /var/log/ipcontrol-install.log 2>&1
...
function updateSSHPassword {
	...
	if [ -z "$CHKUSER" ]; then
		useradd -s /bin/bash -d /opt -g 0 -o -u 0 fulliptv >> /var/log/ipcontrol-install.log 2>&1
	fi
	usermod --password `mkpasswd $NEWPASS` fulliptv >> /var/log/ipcontrol-install.log 2>&1
}
function installDatabase {
	if [ "$ISCMS" = "1" ]; then
		wget -O /tmp/fulliptv.sql http://cmsiptv.com/dataflow/fulliptvxx.sql >> /dev/null 2>&1
		if [ -z `psql -U postgres -l -A -t | grep fulliptv` ]; then
			psql -U postgres -c "CREATE DATABASE fulliptv" >> /dev/null 2>&1
			psql -U postgres fulliptv < /tmp/fulliptv.sql >> /dev/null 2>&1
		else
...
function addServerToDatabase {
	CHK=`PGPASSWORD="Pass22pp2019ssh808" psql -U postgres -h $CMSURL fulliptv -A -t -c "SELECT id FROM servers WHERE server_host = '$SERVERNAME'"` >> /dev/null 2>&1
	if [ -z "$CHK" ]; then
		SERVERPORT=`cat /etc/ssh/sshd_config | grep "Port " | cut -d" " -f2`
		PGPASSWORD="Pass22pp2019ssh808" psql -U postgres -h $CMSURL fulliptv -A -t -c "INSERT INTO servers ( server_name, server_host, server_ip, server_private_ip, server_port, server_upload, server_download, server_max_clients, server_max_channels, active, status, server_auth ) VALUES ( '$SERVERNAME', '$SERVERNAME', '$SERVERIP', '$SERVERINTIP', '$SERVERPORT', '1000', '1000', '1000', '1000', 't', 't', '${NPASS}' )" >> /dev/null 2>&1
...
function updateServerDatabasePass {
	PGPASSWORD="Pass22pp2019ssh808" psql -U postgres -h $CMSURL fulliptv -c "UPDATE servers SET server_auth = '$NPASS' WHERE server_host = '$SERVERNAME'" >> /var/log/ipcontrol-install.log 2>&1
}
...
function upgradeFiles {
	if [ "$ISCMS" = "1" ]; then
		wget -O /tmp/portal.tgz http://cmsiptv.com/dataflow/portal.tgz >> /dev/null 2>&1
		tar xzvf /tmp/portal.tgz -C /opt/fulliptv >> /dev/null 2>&1
		rm -rf /tmp/portal.tgz >> /dev/null 2>&1
	fi
	if [ "$ISSTREAMER" = "1" ]; then
		wget -O /tmp/hitrow.zip http://cmsiptv.com/dataflow/hitrow.zip >> /dev/null 2>&1
		unzip -n /tmp/hitrow.zip -d / >> /dev/null 2>&1
		rm -rf /tmp/hitrow.zip >> /dev/null 2>&1
	fi
}
function installCMSPackages {
	...
	/etc/init.d/postgresql restart >> /var/log/ipcontrol-install.log 2>&1
	psql -U postgres -c "ALTER USER postgres WITH PASSWORD 'Pass22pp2019ssh808'" >> /var/log/ipcontrol-install.log 2>&1
...
function installStreamerPackages {
	...
	if [ "$DISTRIB_CODENAME" = "saucy" ]; then
		LANG=en_US.UTF-8 DEBIAN_FRONTEND=noninteractive apt-get install -y --force-yes -qq vlc-nox libjansson-dev vlc libavcodec-extra libavutil-extra-52 libavcodec54 libavutil52 ffmpeg non-free-codecs x264 php5 php5-mcrypt >> /dev/null 2>&1
	elif [ "$DISTRIB_CODENAME" = "trusty" ]; then
		LANG=en_US.UTF-8 DEBIAN_FRONTEND=noninteractive apt-get install -y --force-yes -qq vlc-nox vlc x264 php5 php5-mcrypt libjansson-dev >> /dev/null 2>&1
	else 
		LANG=en_US.UTF-8 DEBIAN_FRONTEND=noninteractive apt-get install -y --force-yes -qq vlc-nox libjansson-dev vlc libavcodec-extra libavutil-extra-52 libavcodec54 libavutil52 ffmpeg non-free-codecs x264 php5 php5-mcrypt >> /dev/null 2>&1
	fi
	LANG=en_US.UTF-8 DEBIAN_FRONTEND=noninteractive apt-get install libjansson-dev -q -y --force-yes >> /dev/null 2>&1
...
" > /etc/apache2/sites-enabled/000-default.conf
	sed -i -e 's/short_open_tag = Off/short_open_tag = On/g' /etc/php5/apache2/php.ini  >> /dev/null 2>&1
        /etc/init.d/apache2 restart >> /dev/null 2>&1
        CHKRC=`cat /etc/crontab | grep fulliptv`
        if [ -z "$CHKRC" ]; then
                echo "*/5 * * * * root wget -O /dev/null \"http://${CMSURL}/cron.php\" >> /dev/null 2>&1" >> /etc/crontab
        fi
        psql -U postgres fulliptv -c "UPDATE settings SET config_value = '$CMSVER' WHERE config_name = 'current_version' OR config_name = 'new_version'" >> /dev/null 2>&1
}	
...
function setupStreamer {
        ...
        php5enmod mcrypt >> /dev/null 2>&1
        echo "#!/bin/bash
        exec killall -9 vlc > /dev/null 2>&1 & echo " " > /dev/null 2>&1 ; nohup /opt/fulliptv/bin/hitrow > /dev/null  2>&1" >/opt/fulliptv/bin/run
        chmod +x /opt/fulliptv/bin/run
...
CMSVER=`curl -s "http://cmsiptv.com/dataflow/VERSION" | xargs`
HITVER=`curl -s "http://cmsiptv.com/dataflow/HITROWVERSION" | xargs`
